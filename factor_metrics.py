...
class FactorMetrics:
    ...
    def _convert_returns_format(self, returns_df):
        """
        将收益率数据从长格式转换为宽格式
        ...
        returns_df : pd.DataFrame
            长格式的收益率数据，包含 STOCK_CODE, END_DATE, MONTHLY_RETURN 列
            
        返回:
        --------
        pd.DataFrame
            宽格式的收益率数据，行为日期，列为股票代码
        """
        # 确保日期列为datetime类型
        returns_df['END_DATE'] = pd.to_datetime(returns_df['END_DATE'])
    ...
    def load_data(self, start_date='2015-01-01', end_date='2024-12-31'):
        ...
        # 加载收益率数据
        print("加载收益率数据...")
        returns_path = os.path.join(self.data_dir, 'all_monthly_returns.pkl')
        with open(returns_path, 'rb') as f:
            returns = pickle.load(f)
        # print(f"收益率数据形状: {returns.shape}")
        
        # 转换收益率数据格式
        if 'MONTHLY_RETURN' in returns.columns:  # 如果是长格式
            print("转换收益率数据格式...")
            returns = self._convert_returns_format(returns)
                    ...
                    if len(common_stocks) > 0:
                        ...
                        if valid_factors.sum() > 0:
                            ...
                            if len(factor_values) >= n_groups:
                                try:
                                    # 计算分位数，处理重复的边界值
                                    quantiles = pd.qcut(factor_values, n_groups, labels=False, duplicates='drop')
                                    ...
                                    print(f"日期 {date} 的分组计算出现警告: {str(e)}")
                                    # 如果出现错误，尝试减少分组数量
                                    n_actual_groups = min(n_groups, len(factor_values.unique()))
                                    if n_actual_groups > 1:
                                        quantiles = pd.qcut(factor_values, n_actual_groups, labels=False, duplicates='drop')
                                        group_ret = pd.Series(return_values.values, index=quantiles).groupby(level=0).mean()
                                        group_ret.name = date
                                        group_returns.append(group_ret)
                                    continue
    ...
    def _calculate_performance_metrics(self, returns_df):
        ...
        """
        # 计算年化收益率（使用几何平均）
        annual_returns = (1 + returns_df).prod() ** (12 / len(returns_df)) - 1
        
        # 计算年化波动率
        annual_vol = returns_df.std() * np.sqrt(12)  # 修改为月度数据的年化
